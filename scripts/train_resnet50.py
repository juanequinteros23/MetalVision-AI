"""
Training script for ResNet50 on NEU Surface Defect Database.
...
print("=" * 80)
print("METALVISION AI - RESNET-50 TRAINING PIPELINE")
print("=" * 80)
...
# 1. DATA GENERATORS (Raw RGB in [0, 255] passed to resnet.preprocess_input)
...
print("\n[3/7] Building ResNet50 Architecture...")
def build_resnet50_model(input_shape=(224, 224, 3), num_classes=4):
    ...
    model = keras.Model(inputs, outputs, name="ResNet50_MetalVision")
...
# 4. CALLBACKS SETUP
best_model_path = MODEL_DIR / 'resnet50_best.keras'
...
print("\n[7/7] Evaluating Best ResNet50 on Test Set...")
...
plt.subplot(1, 2, 1)
plt.plot(combined_history['accuracy'], label='Train Accuracy', color='#2ca02c', lw=2)
plt.plot(combined_history['val_accuracy'], label='Val Accuracy', color='#d62728', lw=2)
...
plt.subplot(1, 2, 2)
plt.plot(combined_history['loss'], label='Train Loss', color='#2ca02c', lw=2)
plt.plot(combined_history['val_loss'], label='Val Loss', color='#d62728', lw=2)
...
print("\n" + "=" * 80)
print("🎉 RESNET-50 TRAINING AND EVALUATION SUCCESSFULLY COMPLETED!")
