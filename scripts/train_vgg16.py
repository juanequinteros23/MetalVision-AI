"""
Training script for VGG16 on NEU Surface Defect Database.
Optimized for transfer learning, stability, and ensemble integration.
"""
import os
import sys
import json
...
MODEL_DIR = BASE_DIR / 'ml_models' / 'vgg16'
...
print("=" * 80)
print("METALVISION AI - VGG-16 TRAINING PIPELINE")
print("=" * 80)
...
# 1. DATA GENERATORS (Raw RGB in [0, 255] passed to vgg16.preprocess_input)
...
# Save class indices
with open(MODEL_DIR / 'class_indices.json', 'w', encoding='utf-8') as f:
...
print("\n[3/7] Building VGG16 Architecture...")
def build_vgg16_model(input_shape=(224, 224, 3), num_classes=4):
    ...
    model = keras.Model(inputs, outputs, name="VGG16_MetalVision")
...
# 4. CALLBACKS SETUP
best_model_path = MODEL_DIR / 'vgg16_best.keras'
...
print("\n[5/7] PHASE 2: Fine-tuning top 8 layers...")
...
print("\n[7/7] Evaluating Best VGG16 on Test Set...")
...
plt.title('VGG16 - Test Set Confusion Matrix')
...
plt.subplot(1, 2, 1)
plt.plot(combined_history['accuracy'], label='Train Accuracy', color='#ff7f0e', lw=2)
plt.plot(combined_history['val_accuracy'], label='Val Accuracy', color='#1f77b4', lw=2)
plt.axvline(x=len(history_p1.history['accuracy'])-1, color='gray', linestyle='--', label='Phase 2 Start')
plt.title('VGG16 - Accuracy History')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.legend()
...
plt.subplot(1, 2, 2)
plt.plot(combined_history['loss'], label='Train Loss', color='#ff7f0e', lw=2)
plt.plot(combined_history['val_loss'], label='Val Loss', color='#1f77b4', lw=2)
plt.axvline(x=len(history_p1.history['loss'])-1, color='gray', linestyle='--', label='Phase 2 Start')
plt.title('VGG16 - Loss History')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
plt.grid(True, alpha=0.3)
...
# Save final copy
final_model_path = MODEL_DIR / 'vgg16_final.keras'
best_model.save(str(final_model_path))
print(f"✓ Final model saved to {final_model_path}")

print("\n" + "=" * 80)
print("🎉 VGG-16 TRAINING AND EVALUATION SUCCESSFULLY COMPLETED!")
print(f"   Test Accuracy: {clf_report['accuracy']*100:.2f}%")
